...
main()
{
  ...
  IFS=' ' read -r -a plugins <<< $(get_tmux_option "@dracula-plugins" "battery network weather host")
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if [ $plugin = "host" ]; then
      IFS=' ' read -r -a colors <<< $(get_tmux_option "@dracula-host-colors" "dark_purple white")
      script="#h"
    fi
